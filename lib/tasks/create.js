...
const self = module.exports = {
  init: (input, flags) => {
    inquirer.prompt(Constants.CREATE_QUESTIONS)
      .then(answers => {
        ...
        log('Instructions:\n')
        log(Chalk.green(`mkdir -p ${Constants.HIVE_LIBS_DIR}${answers.name}`))
        log(`info.json`)
        
        const infoContent = JSON.stringify(result, null, '  ');

        self.createFileIn(`${Constants.HIVE_LIBS_DIR}${answers.name}`, infoContent)
  ...
  getFormatedResult:(answers => {
    ...
    result.compileType = answers.compileType;
